...
There is no `相关度` field, no topical-relevance score, and no future Project × Opportunity matching layer in the V4 scope.
...
`discovering-backlinks` is the remaining core V4 capability.
...
CrUX remains an optional future evidence source only if real screening usage proves that it adds useful information. It is not part of the core V4 gap.
...
Persistence is therefore not a remaining core V4 implementation gap.

## 11. What V4 Explicitly Does Not Build
...
Therefore the current BacklinkOS V4 development focus is straightforward:
