...
def verify_cdp_live_target(
    ...
    """真实向 CDP 9222 端口查询活跃标签页列表，比对 target_id 与 expected_url (落实 F2 规范)。"""
...
def init_cycle_state(
    ...
    state = {
        ...
        # 持久化已扫描/已处理候选集合，杜绝因行顺序变动漏掉候选 (修复 R7)
...
def plan_next_batch(
    ...
    if sheets_service and commit_prep:
        ...
        try:
            rem_muts = recover_pending_master_mutations(
                sheets_service=sheets_service,
                spreadsheet_id=state["spreadsheet_id"],
    ...
    processed_set = set(state.get("processed_candidate_bids", []))
    processed_set.update(state.get("completed_items", {}).keys())
    processed_set.update(state.get("human_pending_items", {}).keys())

    remaining_bids = [bid for bid in snapshot_bids if bid not in processed_set]
    ...
    # 优先级分组 (严格在 remaining_bids 快照范围内):
    ...
    # 若切片在 Sheet 中已无待提交行，标记为已处理并继续下一批
    ...
    # 沉淀本地细粒度筛选账本 (scan_ledger.jsonl)
    scan_entries = batch_prep_res.get("scan_ledger_entries") or []
    if scan_entries:
        try:
            ledger_file = get_cycle_dir(proj, runtime_dir) / "scan_ledger.jsonl"
    ...
    if commit_prep and sheets_service:
        ...
        if updates:
            ...
            save_pending_master_mutations(proj, unresolved_mutations, runtime_dir)
...
def record_task_outcome(
    ...
    elif len(state.get("processed_candidate_bids", [])) >= state["snapshot_total_count"] and state["active_batch"] is None:
        state["is_finished"] = True
...
def print_cycle_summary(state: dict[str, Any]) -> bool:
    ...
    print(f"初始候选快照总数:    {snap_total}")
    print("--------------------------------------------------")
    print(f"✅ 本轮新增成功提交: {succ}")
    print(f"ℹ️  查重既往已有提交: {prior}")
    print(f"⚠️  不适用 (排除/限制): {not_app}")
    print(f"❌ 失败 (死站/无解):  {failed}")
    print(f"⏸️  待人工处理 (挂起): {hp}")
    print(f"⏳ 仍待提交 (未扫描): {still}")
    print("--------------------------------------------------")
    audit_sum = succ + prior + not_app + failed + hp + still
    is_balanced = (audit_sum == snap_total)
    balance_flag = "对账一致 (BALANCED)" if is_balanced else "⚠️ 账目差异 (UNBALANCED)"
    print(f"账目对账校验: {snap_total} == ({succ} + {prior} + {not_app} + {failed} + {hp} + {still}) -> {balance_flag}")
    if not is_balanced:
