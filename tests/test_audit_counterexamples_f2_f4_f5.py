...
class AuditCounterexamplesTests(unittest.TestCase):
    ...
    def test_f2_uses_explicit_local_cdp_url_for_retained_target(self):
        class CDPFixture(BaseHTTPRequestHandler):
            def do_GET(self):
                body = json.dumps([{"id": "RETAINED_TARGET", "type": "page", "url": "https://example.com/submit"}]).encode()
                self.send_response(200)
                self.send_header("Content-Length", str(len(body)))
                self.end_headers()
                self.wfile.write(body)
        ...
        server = ThreadingHTTPServer(("127.0.0.1", 0), CDPFixture)
        threading.Thread(target=server.serve_forever, daemon=True).start()
        previous = os.environ.get("BACKLINK_BROWSER_CDP_URL")
        os.environ["BACKLINK_BROWSER_CDP_URL"] = f"http://127.0.0.1:{server.server_port}"
        try:
            ok, reason = cycle.verify_cdp_live_target(
                "RETAINED_TARGET",
            ...
            if previous is None:
                ...
                os.environ["BACKLINK_BROWSER_CDP_URL"] = previous
            server.shutdown()
            server.server_close()
