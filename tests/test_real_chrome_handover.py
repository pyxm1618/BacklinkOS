...
class RealChromeHandoverTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # 启动本地 HTTP 测试服务器
        cls.http_port = 18922
        cls.server = socketserver.TCPServer(("127.0.0.1", cls.http_port), SimpleHandler)
        cls.server_thread = threading.Thread(target=cls.server.serve_forever, daemon=True)
        cls.server_thread.start()

        # 启动真实 Chrome 实例 (连接 9222 端口，独立隔离 profile)
        ...
        if not cls.live_target_id:
            ...
            raise RuntimeError("未能启动并连接真实 Chrome CDP 9222 活跃页面")
