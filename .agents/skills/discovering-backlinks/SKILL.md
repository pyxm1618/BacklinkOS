...
3. **硬黑名单直接来自外链总表与跨项目同步。** 若发现 domain 查询显示 `基础状态=已排除` 或 `基础状态=失效`，则视为历史已知硬负例：不重复研究，不重新进入项目执行队列，不覆盖排除原因，记录历史事实即可。不再维护第二套独立黑名单数据库。当确认平台级全局不可用事实（已排除/失效）后，允许有限跨项目同步其他项目中尚未开始的“待提交”记录，写前重新核实，绝不覆盖非待提交历史行，绝不跨项目复制项目专属数据；项目限制（如 AI-only）与付费-only 绝不作为全局排除传播。
